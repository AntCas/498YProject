...
void run_experiment_1a (bool do_rts, int cbr, int seconds, std::string log_name) {
  ...
  positionAlloc->Add(Vector(d,d,0.0)); // A
  positionAlloc->Add(Vector(d,-d,0.0)); // B
  positionAlloc->Add(Vector(-d,-d,0.0)); // C
  positionAlloc->Add(Vector(d,-d,0.0)); // D
  mobility.SetPositionAllocator(positionAlloc);
